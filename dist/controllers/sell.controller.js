...
export const createSell = async (req, res) => {
    try {
        ...
        const itemData = {
            productId: sell.get('id'),
            userId: sell.get('userId'),
            tractoreName: sell.get('title'),
            modelNumber: sell.get('modelYear'),
            tractoreImage: Array.isArray(sell.get('images')) && sell.get('images').length > 0
                ? sell.get('images')[0]
                : undefined,
            tractorType: sell.get('productType'),
            tractorStatus: sell.get('status'),
            price: sell.get('price'),
            location: sell.get('location'),
            brand: sell.get('brand'),
            owner: sell.get('owner'),
            description: sell.get('description'),
            images: sell.get('images'),
            videoUrl: sell.get('videoUrl'),
            createdAt: sell.get('createdAt'),
            updatedAt: sell.get('updatedAt'),
        };
