...
export const createProduct = async (req: Request, res: Response) => {
  try {
    ...
    const imageRecords = await Promise.all(
      imageUrls.map((url: string) => ProductImage.create({ productId, url }))
    );
    // Attach images to response for compatibility
    const prodWithImages: any = product.toJSON();
    prodWithImages.images = imageRecords.map((img: any) =>
      img.getDataValue("url")
    );
...
export const getAllProducts = async (_req: Request, res: Response) => {
  try {
    ...
    const result = products.map((p) => {
      const obj = p.toJSON();
      (obj as any).images = ((obj as any).images || []).map(
        (img: any) => img.url
      );
      return obj;
    });
